 ...
 * bellajs
 * v6.0.3
 * built: Wed, 09 Nov 2016 02:46:00 GMT
...
E=function(a,c,b){c=void 0===c?2:c;a=String(a);return a.length>=c?a:Array(c-a.length+1).join(void 0===b?"0":b)+a},F=function(a,c){c=void 0===c?"":c;for(var b=[];10>b.length;)b.push(Math.random().toString(36).slice(2));var b=b.join(""),d=b.length;for(a=Math.max(a||32,c.length);c.length<a;)c+=b.charAt(Math.floor(Math.random()*d))||"";return c},G=function(a,c){if(!a||0>a)a=0;c||(c=9007199254740991);if(a===c)return c;a>c&&(a=Math.min(a,c),c=Math.max(a,c));return Math.floor(Math.random()*(c-a+1))+a},q=
...
n(a)?Object.keys(a).forEach(function(c){d[c]=a[c]}):d[a]=b;return q(d)}});return c};return function(b){return k(b)?a(b):n(b)?c(b):b}}(),J=function(){for(var a=[],c=0;64>c;)a[c]=0|4294967296*Math.abs(Math.sin(++c));return function(c){var b,d,e,l,x=[];c=unescape(encodeURI(c));for(var h=c.length,f=[b=1732584193,d=-271733879,~b,~d],p=0;p<=h;)x[p>>2]|=(c.charCodeAt(p)||128)<<8*(p++%4);x[c=16*(h+8>>6)+14]=8*h;for(p=0;p<c;p+=16){h=f;for(l=0;64>l;)h=[e=h[3],(b=h[1]|0)+((e=h[0]+[b&(d=h[2])|~b&e,e&b|~e&d,b^
d^e,d^(b|~e)][h=l>>4]+(a[l]+(x[[l,5*l+1,3*l+5,7*l][h]%16+p]|0)))<<(h=[7,12,17,22,5,9,14,20,4,11,16,23,6,10,15,21][4*h+l++%4])|e>>>32-h),b,d];for(l=4;l;)f[--l]=f[l]+h[l]}for(c="";32>l;)c+=(f[l>>3]>>4*(1^l++&7)&15).toString(16);return c}}(),w=function(a,c,b){e(a)&&(a=String(a));if(!a||!d(a))return"";e(c)&&(c=String(c));e(b)&&(b=String(b));if(d(c)&&d(b))a=a.split(c).join(b);else if(k(c)&&d(b))c.forEach(function(c){a=w(a,c,b)});else if(k(c)&&k(b)&&c.length===b.length){var f=c.length;if(0<f)for(var g=
...
n(b)||k(b))?e(a,b):a},md5:J,createId:F,random:G,stabilize:q,now:y,time:I,date:K}});